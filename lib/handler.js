...
if ( STRIPE_KEY === undefined || STRIPE_KEY === null || STRIPE_KEY === "undefined" ) {
...
module.exports.createguest = function(event, context, callback) {
	...
	return;
    } else if ( guest.agegroup != 'ADULT_TICKET' && guest.agegroup != 'HIGHSCHOOL_TICKET' && guest.agegroup != 'PREK_TICKET' && guest.agegroup != 'CHILD_TICKET' && guest.agegroup != "JUNIORHIGH_TICKET" && guest.agegroup != "HIGH_VOLUNTEER_TICKET" && guest.agegroup != "JR_VOLUNTEER_TICKET" && guest.agegroup != "ABSENTEE_TICKET" ) {
    ...
    var put_user = function() {
        ...
        if ( guest.agegroup != 'ADULT_TICKET' && guest.agegroup != "ABSENTEE_TICKET" ) {
...
function send_invoice(req, callback,msg, subject, do_charge) {
	...
	for(var customerid in customermap) {
		...
		for(var idx in customer_to_bidnums[customerid]) {
			...
			message += "<tr><td>" + item.buyername + "</td><td>" + item.name + "</td><td>" + item.donor.firstname + " " + item.donor.lastname +  "</td><td>" + item.donor.email + "</td><td>$" + item.sellprice + "</td></tr>";
		    }
		}
...
module.exports.ChargeCustomer = function(event, context, callback) {
	...
	var deferred = new Promise(function(resolve, reject) {
		...
		return;
	    } else if ( guest.agegroup != 'ADULT_TICKET' && guest.agegroup != 'HIGHSCHOOL_TICKET' && guest.agegroup != 'CHILD_TICKET'  && guest.agegroup != 'PREK_TICKET' && guest.agegroup != "JUNIORHIGH_TICKET" && guest.agegroup != "HIGH_VOLUNTEER_TICKET" && guest.agegroup != "JR_VOLUNTEER_TICKET" && guest.agegroup != "ABSENTEE_TICKET" ) {
              console.log(guest.agegroup);
	...
	console.log(customer);
      ...
      let finish_up = function(chargeId) {
        if ( cust_err ) {
          callback(null, {
            statusCode: 300,
...
function computeHash(password, salt, fn) {
    crypto.pbkdf2(password, salt, 1000, 64, 'sha1', function(err, derivedKey) {
	if (err) return fn(err);
